...
// Smooth scrolling to targets for main menu links
    $('a[href^="#"]').click(function(){
        ...
        $('html, body').animate(
            {
                scrollTop: $(target).offset().top - 80/1920*100 // subtract the main menu height to see all the section content
            },
            800
        );
    });

// Fix header on scrolling the page down
    let menu_selector = "header";
    let offset = $(menu_selector).offset();
    let scroll_top = $(window).scrollTop();

    $(window).scroll(function() {
        if ( $(window).scrollTop() > offset.top ) {
            $(menu_selector).addClass('fixedNav');
        } else {
            $(menu_selector).removeClass('fixedNav');
        }

        // Change an active menu item on scrolling the page    
...
// Work with LOAD MORE button on the About Us section
    $('#about_read_more_btn').click(function() {
        // show or hide additional text
        $('#about_read_more_content').toggleClass('open');
        // and change button text
        $('#about_read_more_btn').text( $('#about_read_more_btn').text().trim() === "read more" ? "read less" : "read more" );        
    });

// Work with LOAD MORE button on the Complaints section
    $('#handling_complaint_read_more_btn').click(function() {
        // show or hide additional text
        $('#handling_complaint_read_more_content').toggleClass('open');
        // and change the button text
        $('#handling_complaint_read_more_btn').text( $('#handling_complaint_read_more_btn').text().trim() === "read more" ? "read less" : "read more" );        
    });

// Work with Read More link on the Safety of Funds section
    $('#safety_of_funds_01_link').click(function() {
        // show or hide more text
        $('#safety_of_funds_01_content').toggleClass('open');
        // and change the link text
        $('#safety_of_funds_01_link').text( $('#safety_of_funds_01_link').text().trim() === "Read More" ? "Read Less" : "Read More" );    
    });

    $('#safety_of_funds_02_link').click(function() {
        // show or hide more text
        $('#safety_of_funds_02_content').toggleClass('open');
        // and change the link text
        $('#safety_of_funds_02_link').text( $('#safety_of_funds_02_link').text().trim() === "Read More" ? "Read Less" : "Read More" );   
    });            
